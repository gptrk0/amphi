...
- [x] **Értesítések** (2026-08-08) — ld. a lenti „Telegram-értesítések" alfejezetet. Telegram Bot API-val indult; **2026-08-17 óta mindkét csatorna webhook URL** (ld. „A telepítés értesítése is webhook lett"), tehát a Discord nem külön implementáció, hanem egy másik URL ugyanabban a mezőben. Böngésző push továbbra is nyitott — az az egy tényleg új `notify` ág, nem átépítés.
...
#### A kérő átírható — a figyelőlistán egy, a gyűjteményben több (2026-08-17-i kérés) ✅ a kódban és a szerveren mérve

Kérés: „watchlistnél és librarynál a táblázatban a user aki kérvényezte az módosítható legyen, kizárólag admin… admin esetén a név helyén egy dropdown legyen ahol kitud választani másik usert (vagy usereket ha indokolt)".

**Az „indokolt" a két táblánál két különböző választ ad, és ezt az adatmodell dönti el, nem a beleérzés.** A `Watchlist` sor **egy** emberé (`userId` idegen kulcs, `@@unique([userId, tmdbId, type])`): a sor *maga* az, hogy valaki akar valamit, tehát ott a több-választás nem egy bővebb mező, hanem egy másik művelet (sorok létrehozása). A `Library.watchedBy` viszont `Int[]`, mert egy letöltés eleve többeknek szól — ott a több-választás maga a mező. Így a figyelőlistán **átadás** van, a gyűjteményben **halmaz-szerkesztés**.

- **Figyelőlista, „Felvette" oszlop**: a meglévő `OptionSelect`, `float`-tal — pont az, ami a nyelv oszlopban már ott van ugyanabban a táblában. Ütközésre (a célszemélynek már van sora ugyanarra a címre) **409** és `conflict: true`, amit a kliens a saját nyelvén mond ki: két lista összeolvasztása nem az app döntése, mindkettőnek saját egységei, előzménye és nyelve van.
- **A keresési létra nullázódik, de csak ha kell.** A sor nyelve gyakran üres, ami azt jelenti, hogy „amit a fiókom mond" — így egy átadás megváltoztathatja, *mit* keresünk, anélkül hogy a sor bármelyik mezőjéhez hozzáérnénk. A `setOwner` ezért összehasonlítja a feloldott nyelveket előtte-utána, és csak eltérésnél nullázza a `searchAttempts`/`lastCheckedAt`/`searchingSince` hármast — ugyanazzal az érveléssel, amiért a `setRequestedLanguage` is nullázza: a mostani backoff mögötti két nap egy olyan kérdésre ment el, amit már senki nem kérdez. Ha nem tér el, a backoff marad, mert a keresésről semmi nem változott.
- **Gyűjtemény, „Kérte" oszlop**: pipálható névlista dialógusban, ugyanabban az alakban, ahogy a „Megőrzés" is szerkesztődik egy sorral arrébb (`OptionCheckboxes`, `max-h-[50vh]` görgetéssel). **Letöltésenként, nem címenként**: az összecsukott cím sora továbbra is csak az uniót mutatja, olvasásra — egy film magyar és angol kiadása két külön embert szolgál ki, ezért nincs egyetlen lista, amit oda be lehetne írni.
- **Nem kozmetika, és a dialógus ki is mondja:** ez a lista az, aki értesítést kap, akinek a figyelőlistájára visszakerül egy meghiúsult letöltés, és akinek a főoldalán megjelenik. Plusz — ez a fele, amin kétszer kell gondolkodni — a `libraryFilter` szerint ez teszi az adott kiadást „megvan"-ná az illetőnek, tehát valakit hozzáadni azt is jelenti, hogy neki nem keressük tovább. A `claimHeldUnits` ezt magától rendezi: a következő körben az ő egységeit ez a letöltés viszi el.
- A `watchers` mellé bekerült a `watcherIds` ugyanabból a szűrésből, hogy a kettő ne mondhasson mást — egy törölt fiók így mindkettőből kiesik, nem pedig névtelen számként jelenik meg egy dropdownban. Az `OptionSelect` `TagOption`-je pedig kapott egy `hint` mezőt (a szürke kód a címke mellett): nyelvnél `hun`, usernél `""`, mert egy sor-id semmit nem jelent az olvasónak, egy e-mail meg nem fér el egy táblacellában. A cím keresőszóként ott van.
- **A jogosultság két helyen áll.** A kliensen a `/api/users` az, ami nem-adminnak nem válaszol, tehát nincs lista, amit felajánlhatna, és mindkét oszlop a régi szöveg marad. A szerveren a `refuseUnlessAdmin` — ez az, ami számít.

**Élő ellenőrzés (hamisított admin session, valódi adaton):** sikeres átadás oda-vissza (`userId` 1 → 3 → 1, a `searchAttempts` 8 maradt, mert a két fiók nyelve ugyanaz), 409 ütközésre, 400 nem létező fiókra és szemétre, **403 nem-adminnak a saját során is**, a `language` ág változatlanul 200. A gyűjteményben: `{3,4}` beírva, üresre törölve (érvényes állapot), 400 nem létező id-ra, nem-listára és `[0,"x"]`-re, a `keepDays` ág érintetlen. A naplósorok olvashatóan állnak össze: `off Patrick's list, with everything still to be found on it — by Patrick`. A teszthez létrehozott sorok törölve. `tsc --noEmit` és `next lint` tiszta. **Amit nem próbáltam:** a nyelv-eltérés miatti nullázó ág — ahhoz egy valódi felhasználó nyelvi profilját kellett volna átírni egy élő installon.

---

#### A telepítés értesítése is webhook lett (2026-08-17-i kérés) ✅ a kódban és a szerveren mérve

Kérés: „a settingsben lévő notification telegram beállítás a user settingshez hasonló webhookos megoldás legyen".

A fiókok 2026-08-09 óta ismerik az általános választ (ld. „Egy URL, két alak"), az install-csatorna viszont maradt az, ami 2026-08-08-án készült: `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` + `TELEGRAM_API_URL` + `TELEGRAM_EVENTS`. **Három mező, ami egy szolgáltatást nevez meg** — és csak az az egy szolgáltatás tudta használni. Most `NOTIFY_WEBHOOK_URL` + `NOTIFY_EVENTS`, ugyanazon a `callWebhook`-on: placeholder az URL-ben → GET a query stringben, placeholder nélkül → POST `{"content": …}` törzzsel.

- **A `notify.ts` fele eltűnt**: nincs többé `api()`, `token()`, `chatId()`, HTML-`escape()`, `body()` és saját `send()` axios-hívással. A `notify()` most `webhookFields`-et épít és `callWebhook`-ot hív — ugyanazt, amit a `notifyUsers` a személyes webhookokra. A régi fájl-fejléc azzal kezdődött, hogy „kétféle címzett, és **már nem** osztoznak mechanizmuson"; ez a mondat fordult meg.
- **Ami megmaradt a különbségből, az a lényegi:** az install-csatorna az egyetlen, ami *mindenkiről* hall, tehát **nevet kap** (`who`), a személyes webhook nem — az egyetlen ember, akiről hall, a tulajdonosa. A `who` ötödik `WebhookFields`-mező és ötödik placeholder lett, **és bele van fűzve a `message`-be is**: a legtöbb URL csak `{message}`-t hordoz (vagy semmit, és akkor a teljes üzenet a JSON törzs), tehát a névnek a mondatban kell lennie, különben az install-csatorna pont azt a felét veszti el, amiért olvasni érdemes. Személyes webhooknál `who` nincs, és a kimenet karakterre ugyanaz, mint eddig.
- **Teszt gomb az admin oldalon is**, ugyanaz, ami a fiók oldalán van, és a mezőben *épp látható* címre küld — hogy az elírás akkor derüljön ki, amíg az űrlap nyitva van, ne három hét múlva abból, hogy nem jött értesítés. A küldés és a szövegek egy helyen élnek (`testWebhookUrl`), mert ugyanazt a kérdést két dialektusban megválaszolni pont az a fajta elcsúszás, amit ez a kódbázis máshol is kerül. A `webhook` i18n-névtér ugyanezért van: a `Teszt` gomb és a „Használd ezt kiindulásnak" tooltip most már két helyen jelenik meg, egy szövegből. A beillesztendő Telegram/Discord példák is egy komponensbe kerültek (`webhook-examples.tsx`) — eddig a fiók oldalán álltak, most mindkét mező alatt ott vannak.
- **Titok, de csak a naplónak.** A `NOTIFY_WEBHOOK_URL` `secret: true`, mert egy Telegram URL-ben a bot token az útvonal része — a napló megnevezi a beállítást, nem idézi. Az érték a 2026-08-10-i döntés szerint látszik az admin oldalon. A `scrub()` `/bot<digits>:<token>` szabálya egyébként eddig is megvolt, tehát ha az URL mégis egy hibaüzenetben landol, maszkolva kerül a táblába.
- **A migráció megépíti az URL-t abból, ami már ott van** (`20260817120000_the_installs_notification_is_a_webhook`), tehát egy működő installnak senki nem gépeli újra a tokent: `<api>/bot<token>/sendMessage?chat_id=<chat>&text={message}` — pontosan az az alak, amit a fiók oldala Telegram-példaként eddig is felajánlott. Csak akkor, ha van miből (token és chat id, mindkettő nem üres); amelyik install soha nem állította be, az nem kap sort és marad „nincs beállítva". Az események értékben mennek át, és a `NOTIFY_EVENTS` defaultja ugyanaz, ami a `TELEGRAM_EVENTS`-é volt, tehát egy soha nem szerkesztett lista viselkedése változatlan. A négy régi sor törlődik: egy sehol nem olvasott token feleslegesen őrzött titok.

**Élő ellenőrzés:** a migráció **előbb próbán**, `BEGIN … ROLLBACK`-ben — mert a `DELETE` visszafordíthatatlan, és egy hibás `INSERT` mellett a valódi bot token veszett volna el. A próba a helyes URL-t építette (`https://api.telegram.org/bot***/sendMessage?chat_id=<a csoport id-ja>&text={message}` — a token a napló `scrub()`-jának szabályával maszkolva, a chat id pedig azért nincs itt, mert egy publikusra szánt repóban semmit nem nyer), utána mentés a két sorról, majd `prisma migrate deploy`: a `TELEGRAM_*` sorok eltűntek, a `NOTIFY_WEBHOOK_URL` a helyén. Újraindulás után a scheduler azt írja, hogy **`the install's notifications are on`** — tehát az `isNotifyConfigured()` a migrált URL-t olvassa. A teszt-végpont mindhárom hibaútja mérve: elérhetetlen host → 502 `ECONNREFUSED`, privát cím → az SSRF-őr saját mondata, nem-URL → 502, és mindhárom írt egy `notify` naplósort a nevemmel. Session nélkül 401. `tsc --noEmit` és `next lint` tiszta. **Amit nem tettem meg:** valódi üzenetet nem küldtem ki — a chat id negatív, tehát csoport, és egy „A test from Amphi" sor egy családi csoportba nem az én döntésem. A Telegram felé vezető utolsó lépés így a settings oldal Teszt gombjára vár.
